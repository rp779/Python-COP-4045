...
A fifth class is added - Human which is a subclass of Animal. Humans can eat only predators and not prey.
Predators do not eat humans. Humans breed the same way that Predators and Preys eat. 
"""


import pylab  # plotting
import time   # for clock tick
import random  # randomly place animals on the Island
...
class Island (object):
    ...
    def clear_all_moved_flags(self):
        ''' Animals have a moved flag to indicate they moved this turn.
...
##############################################################################
#              the Human class added for h3 p3                               #
##############################################################################
...
def main(human_breed_time=3, human_starve_time=5, initial_human=1, predator_breed_time=6, predator_starve_time=9, initial_predators=10, prey_breed_time=3, initial_prey=50, size=20, ticks=500):
...
#        ans = input("Return to continue")
    ...
    print(prey_list)
    print(human_list)
    print('First tick: {}'.format(1))
    print('Last tick: {}'.format(i))

    pylab.plot(range(0, i), predator_list, label="Predators")
    pylab.plot(range(0, i), prey_list, label="Prey")
    # added the functionality to print the Human instances over clock ticks
    pylab.plot(range(0, i), human_list, label="Humans")
